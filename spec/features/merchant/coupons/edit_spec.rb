...
RSpec.describe "As a merchant", type: :feature do
  it "can edit a coupon" do
    ...
    expect(current_path).to eq(edit_merchant_dash_coupon(coupon))

    expect(find_field('Name').value).to eq coupon.name
    expect(find_field('Code').value).to eq coupon.code
    expect(find_field('Percent').value).to eq coupon.percent
    expect(find_field('Active').value).to eq coupon.active
